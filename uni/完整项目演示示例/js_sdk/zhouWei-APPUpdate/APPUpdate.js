...
export const getServerNo = function(version, isPrompt = false, callback) {
	 ...
	 */
	httpData["model"] = "Data/UpdateApp";
	httpData['token'] = uni.getStorageSync("userInfo").token;
...
function updatePopup(data, callback) {
	...
	const txtLen =  data.versionInfo.split("|");
	...
	// 文本高度
	let viewContentHeight = txtLen.length == 1 ? 80 : 40 + txtLen.length * 20;
	// 弹窗容器高度
...
export function APPUpdate(isPrompt = false) {
	getCurrentNo(version => {
		getServerNo(version.versionCode, isPrompt, res => {
			if (res.forceUpdate) {
				if (/\.wgt$/i.test(res.downloadUrl)) {
