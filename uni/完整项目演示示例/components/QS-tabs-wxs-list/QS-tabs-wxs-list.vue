						...
						<swiper id="swiper" class="swiper" :style="{
						 ...
						 @animationfinish="QSTABSWXS.animationfinish">
							<swiper-item v-for="(item, index) in tabs" :key="index" class="swiper-item" :style="{
								...
								<QSTabsListTemplate :ref="refPre" :url="url" :readyRefresh="getCurrent===index?Boolean(readyRefresh):false" :show="readyRefresh?
									(getCurrent===index):
					...
					'height': getSwiperHieght + 'px'
					 ...
					 @animationfinish="QSTABSWXS.animationfinish">
						...
						'background-color': item.swiperItemBackgroundColor || 'rgba(255,255,255,0)'
							...
							<QSTabsListTemplate :ref="refPre" :url="url" :readyRefresh="getCurrent===index?readyRefresh:false" :show="readyRefresh?
							(getCurrent===index):
							(((index-getCurrent)<=1 && (index-getCurrent)>=-1)?
							true:false)"
							 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus" @scrollFn="scrollFn"></QSTabsListTemplate>
						...
						<view class="vShowView" :style="{ 'height': getSwiperHieght + 'px' }">
							<view class="vShow-item" v-for="(item, index) in tabs" :class="{
							 ...
							 :key="index" :style="{ 
								...
								<QSTabsListTemplate :ref="refPre" :url="url" :readyRefresh="getCurrent===index?readyRefresh:false" :show="getCurrent===index"
								 :type="type" :current="getCurrent" :tab="item" :index="index" @refreshEnd="setRefreshStatus" @scrollFn="scrollFn"></QSTabsListTemplate>
							</view>
						</view>
					</view>
