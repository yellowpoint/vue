<template>
	<layout :loading="loading" pageKey="user" title="用户列表">
		...
		<block slot="titleRight">
			<auth-nav href="/pages/user/userEdit" url="admin/user/info" class="btn w80 flex">
		...
		<tables :list="page.list">
			...
			<block slot="tbody">
				<tr v-for="( x , index) in page.list" :key="index">
					...
					<td>
						<view class="flex lt">
							<files :list="x.avatar" :isRound="true"></files>
							{{x.realNameAuth ? x.realNameAuth.contact_person : ''}}
						</view>
					</td>
					<td>{{x.mobile}}</td>
					<td>{{x.roleNames}}</td>
					...
					<td>
						<switch-btn :disabled="x.isSuperAdmin" url="admin/user/changeStatus" :params="{id:x._id}" :checked="x.status == 0" @change="statusChanged"></switch-btn>
					</td>
					<td>
						<auth-nav :href="'/pages/user/userEdit?id=' + x._id" class="plr5 bold main" url="admin/user/info">
							编辑
						</auth-nav>
						<auth-btn v-if="!x.isSuperAdmin" :url="'admin/user/delete?id='+x._id" confirm="delete" @success="remove(index,1)" class="plr5 bold main">
							删除
						</auth-btn>
					</td>
				</tr>
