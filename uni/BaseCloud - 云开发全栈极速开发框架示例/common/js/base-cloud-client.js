...
var LOCAL_CACHE = {} ;
function call({ url, data = {}, success, fail, complete , debug = true , cacheKey , cacheSpace , keepStr = false , keepKeys }) {
	...
	if (isDebug(debug) ) {
		console.log("request======================>>");
		console.log(`开始请求，云函数： ${name} , action ： ${action} , data ：` + JSON.stringify(data));
		console.log( data );
	...
	uniCloud.callFunction({
		...
		fail: (e) => {
			if ( isDebug(debug) ) {
				console.log("http请求失败：" + url + " -> requestId:" , e.requestId);
				console.log(e);
			}
			uni.showToast({
				title: '请求失败',
...
function callInCache({url , data , success , fail , complete , debug = true , keepStr = false , keepKeys}){
...
function dealDataByType(data , keepKeys) {
	...
	for (var key in data) {
		...
		if (isNaN(parseFloat(value)) && (value == 'true' || value == true || value == 'false' || value == false)) {
			console.log("...");
...
function dealResult(res , dataset , success , fail , complete ){
	uni.hideLoading();
...
function submitData( dataset , data , success , fail , complete){
	...
	call({
		...
		data : data ,
		success : res =>{
			dealResult(res , dataset , success , fail , complete);
		},
		fail : res =>{
			dealResult(res , dataset , success , fail , complete);
		},
