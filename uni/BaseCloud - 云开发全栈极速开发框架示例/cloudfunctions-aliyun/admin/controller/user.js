...
module.exports = {
	...
	login : async function(e){
		var {username , password} = this.event.params ;
		var res = await this.uniID.login({
			username , 
			password : password + ""
		});
		return !res.code ? {...res , state : 'ok'} : this.fail(res.msg);
	},
	...
	myInfo : async function({user}){
		var data = {
			...
			realNameAuth : {
				contact_person : user.realNameAuth ?  user.realNameAuth.contact_person : "" 
			},
			mobile : user.mobile ,
	...
	modify:async function({user}){
		var data = this.getModel("x" , "password,mobile,comment,avatar");
	...
	list : async function(res){
		...
		var page = await this.paginate({
			...
			pageSize , 
			collection : User ,
			where : ( dbCmd.or( {isSuperAdmin : true } , { roleIds : dbCmd.exists(true) } ) ),
			eq:["status","name"] ,
			field : {
				"avatar" : true ,
				"username" : true ,
				...
				"roleNames" : true ,
				"status" : true ,
				"realNameAuth.contact_person" : true 
			},
