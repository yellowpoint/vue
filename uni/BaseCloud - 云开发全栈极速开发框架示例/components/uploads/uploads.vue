<template>
	<labels :title="title" :titleWidth="titleWidth" :isTop="isTop" :isVertical="isVertical">
		<view class="clear father" style="left: -5px;">
			...
			<files :type="type" :valueKey="valueKey" class="fl" :list="fileList" :width="80" :count="count" @remove="remove" :disabled="false"></files>
			
			<view class="w80 fl pd5" @tap="chooseFile" v-if="list.length < count || count == -1">
				<view class="square">
...
<script>
	...
	export default {
		...
		props: {
			...
			type: { //image、audio、video、pdf、apk
				type: String ,
				default: "image" 
			},
			maxDuration : {
		...
		computed: {
			curValue: function(e) {
				return this.valueKey ? JSON.stringify(this.fileList) : this.list.join(",");
			}
		},
		watch: {
			value: function(e) {
		...
		methods: {
			...
			getFileData: function(file, fileId) {
				var fileData = {
					...
					name: file.name,
					type: file.type,
					extends: this.getExtension(file),
					path: file.path,
